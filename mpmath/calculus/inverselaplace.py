...
#            self.degree = max(12,int(1.38*self.dps_goal))
            self.dps_goal = int(172*self.ctx.dps/100)
            self.degree = max(12,int(138*self.dps_goal/100))
...
# change DH #
        ...
        self.theta = self.ctx.linspace(0, 1*self.ctx.pi, M+1)
        ...
        **References**
        ...
        for i in range(1,M):
            ans[i] = self.ctx.exp(delta[i])*fp[i]*(
                1 + _1j*theta[i]*(1 + self.cot_theta[i]**2) -
                _1j*self.cot_theta[i])
...
class deHoog(InverseLaplaceTransform):
    ...
    def calc_laplace_parameter(self, t, **kwargs):
        ...
        self.p = (self.gamma + 1*self.ctx.pi*
                  self.ctx.matrix(self.ctx.arange(self.np))/self.T*_1j)
