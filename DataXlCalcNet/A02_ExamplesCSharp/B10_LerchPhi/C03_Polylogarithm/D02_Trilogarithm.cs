...
#region TestTrilogReal

public static void TestTrilogReal()
{
    Console.WriteLine("<H1 Title=" + "\"" + "TestTrilogReal" + "\"" + ">");
    double[] InputArray1 = { -0.333, 0.0, 4.333 };
    
    foreach (var x in InputArray1) {
        Console.WriteLine("<H2 Title=" + "\"" + "trilog(x); " + "x={0}" 
            + "\"" + ">", x);
        Double res01 = math53.trilog(x);
        Console.WriteLine("math53:  {0}", res01);
#if HasArbPrecNet
/* No syntax highlighting if HasArbPrecNet is undefined */
        Single res08 = sflint.trilog(x);
        Console.WriteLine("sflint:  {0}", res08);
        Double res09 = dflint.trilog(x);
        Console.WriteLine("dflint:  {0}", res09);
        Extended res10 = eflint.trilog(x);
        Console.WriteLine("eflint:  {0}", res10);
        Quadruple res11 = qflint.trilog(x);
        Console.WriteLine("qflint:  {0}", res11);
        Octuple res12 = oflint.trilog(x);
        Console.WriteLine("oflint:  {0}", res12);
        Mpfr res13 = mflint.trilog(x);
        Console.WriteLine("mflint:  {0}", res13);
        Arb res16 = aflint.trilog(x);
...
#region TestTrilogCplx

public static void TestTrilogCplx()
{
    Console.WriteLine("<H1 Title=" + "\"" + "TestTrilogCplx" + "\"" + ">");
    Complex[] InputArray1C = { dcplx.t(-4.333d, 2), dcplx.t(0.0d, 2), 
        dcplx.t(4.333d, 2) };
    foreach (var x in InputArray1C) {
        Console.WriteLine("<H2 Title=" + "\"" + "trilog(x); " + "x={0}" + 
            "\"" + ">", x);
        Complex res01 = cmath53.trilog(x);
        Console.WriteLine("cmath53:  {0}", res01);
#if HasArbPrecNet
/* No syntax highlighting if HasArbPrecNet is undefined */
        SingleC res08 = sflintc.trilog(x);
        Console.WriteLine("sflintc:  {0}", res08);
        Complex res09 = dflintc.trilog(x);
        Console.WriteLine("dflintc:  {0}", res09);
        ExtendedC res10 = eflintc.trilog(x);
        Console.WriteLine("eflintc:  {0}", res10);
        QuadrupleC res11 = qflintc.trilog(x);
        Console.WriteLine("qflintc:  {0}", res11);
        OctupleC res12 = oflintc.trilog(x);
        Console.WriteLine("oflintc:  {0}", res12);
        MpfrC res13 = mflintc.trilog(x);
        Console.WriteLine("mflintc:  {0}", res13);
        ArbC res16 = aflintc.trilog(x);
...
#region TestTrilogRealImag

public static void TestTrilogRealImag()
{
    Console.WriteLine("<H1 Title=" + "\"" + "TestTrilogRealImag" + 
        "\"" + ">");
