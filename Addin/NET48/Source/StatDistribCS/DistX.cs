...
namespace NewDistrib
{
    ...
    public static class DistX
    {
        ...
        public static void BrentDouble(bool UseLeftTail, bool IsExact, bool IsGLM, int proc, ref double a, ref double b, double fa, double fb, double t1, double LogTarget, double Df1, double Df2, double omega)
        {
            ...
            while (iter < maxiter)
            {
                ...
                switch (proc)
                {
                    default:
                        {
                            // Case mp_cdisx : LogRefTail = DistFromBoost.Arb_ChiSquare_CDF(b, Df1, UseLeftTail, True).ToDouble
                            // Case mp_fdisx : LogRefTail = DistFromBoost.Arb_F_CDF(b, Df1, Df2, UseLeftTail, True).ToDouble
        ...
        public static double ndisx1(double LeftTailTarget, double RightTailTarget)
        {
            ...
            double r;
            double r0; // or apr_t
            double Q = LeftTailTarget - 0.5d;  // calculation done in apr_t, result converted back to double
            if (Q < 0d)
                r0 = LeftTailTarget;
            else
                r0 = RightTailTarget;
            ...
            else
            {
                ...
                r = Math.Sqrt(-Math.Log(r0));  // calculation done in apr_t, result converted back to double
                if (r <= split2)
                {
                    r = r - const2;
        ...
        // L = 6 * (z * stdev + mean)
        // h = xpr.cbrt(2 * (L + Math.Sqrt(13 + L * (L - 5))) - 5)
        // u = 0.5 + 0.5 * h - 1.5 / h
        // u = u * u * u
        // result = n * u * u
        // End If
        ...
        public static double cdisx(double LeftTail, double RightTail, double Df1)
        {
            ...
            x1 = boost2.dist_chisq(LeftTail, Df1, 6d);
            // Else
        ...
        public static double fdisx(double LeftTail, double RightTail, double Df1, double Df2)
        {
            ...
            if (LeftTail < 0.5d)
            {
                x1 = boost2.dist_fisher_f(LeftTail, Df1, Df2, 6d);
            }
            else
            {
                x1 = boost2.dist_fisher_f(RightTail, Df1, Df2, 7d);
            }
            Console.WriteLine("x1: {0}", x1);
        ...
        public static void demoNdisx()
        {
            ...
            Console.WriteLine("");
            // Dim R2 As Double = xpr.dist_qnorm(LeftTail, 0, 1, True, False)
        ...
        public static void demoCdisx()
        {
            ...
            double L1 = boost2.dist_chisq(X0, m, 2d);
            Console.WriteLine("L1: {0} ", L1);
            double R1 = boost2.dist_chisq(X0, m, 3d);
            Console.WriteLine("R1: {0} ", R1);
            Console.WriteLine("");
        }
        ...
        public static void demoFdisx()
        {
            ...
            Console.WriteLine("X0: {0} ", X0);
            double L1 = boost2.dist_fisher_f(X0, m, n, 2d);
            Console.WriteLine("L1: {0} ", L1);
            double R1 = boost2.dist_fisher_f(X0, m, n, 3d);
            Console.WriteLine("R1: {0} ", R1);
        ...
        public static void demoTdisx()
        {
            ...
            Console.WriteLine("R1: {0} ", R1);
            Console.WriteLine("");
            // Dim R2 As Double = xpr.dist_qt(LeftTail, m, True, False)
            // Console.WriteLine("R2: {0} ", R2)
            Console.WriteLine("");
        }
        ...
        public static void demoBetadisx()
        {
            ...
            Console.WriteLine("");
            // Dim x1 = xpr.dist_qbeta(LeftTail, a, b, True, False)
            // Dim y1 = xpr.dist_qbeta(RightTail, a, b, False, False)
