...
def main_tests():
    demo_real_gamma_p_inva()


def demo_real_gamma_p_inva():
    print('demo_real_gamma_p_inva: ')
    xlist = [1.5, 3.5]
    plist = [0.1, 0.8]
    for x in xlist:
        for p in plist:
            for ctx in ctxlist:
                ...
                res = ctx.real_gamma_p_inva(x, p)
                print(name, ': gamma_p_inva(x=' + str(x) + ', p=' + str(p),'): ' + ctx.fmt(res))
            print()
        print()
