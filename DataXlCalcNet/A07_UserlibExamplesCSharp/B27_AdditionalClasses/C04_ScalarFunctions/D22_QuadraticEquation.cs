...
/* No syntax highlighting if none of the above is defined */
    ArbPrec.SetDps(40);
#endif
    DemoSolveQuadraticReal();
    DemoSolveQuadraticCplx();
}


#region DemoSolveQuadratic


public static void DemoSolveQuadraticReal()
{
    Console.WriteLine("Hello DemoSolveQuadraticReal: " + Ctx.name);

    var A = 1;
    var B = 1.21025607898502;
    var C = 0.257707596061509;

    var res = Ctx.quadratic_equation(A, B, C);
    Console.WriteLine("x1: {0}, f(x1): {1}", res.Item1, Ctx.eval_quadratic(res.Item1, A, B, C));
    Console.WriteLine("x2: {0}, f(x2): {1}", res.Item2, Ctx.eval_quadratic(res.Item2, A, B, C));
}

public static void DemoSolveQuadraticCplx()
{
    Console.WriteLine("Hello DemoSolveQuadraticCplx: " + Ctx.name);

    var A = dcplx.t(1, 0);
    var B = dcplx.t(-0.400848385532961, 0.166133732712128);
    var C = dcplx.t(0.531405903663382, -0.844183967201696);
