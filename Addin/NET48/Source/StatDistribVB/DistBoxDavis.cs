    ...
    static class DistBoxDavis
    {
        ...
        public static void BoxFApprox(string result, double f1, ref double m, double omeg1, double omeg2, ref double LeftTail, ref double RightTail)
        {
            ...
            if (result == "PValue")
            {
                ...
                LeftTail = NewDistrib.DistN.Fdisn2(f1, f2, x, 0d, ref LeftTail, ref RightTail);
            }
            else
            {
                x = NewDistrib.DistX.fdisx(f1, f2, LeftTail, RightTail);
                if (C > 0d)
                    m = x * b;
                else
                    m = b / (f2 / (f1 * x) + 1d);
        ...
        public static void BetaProdDis2(int p, double[] b, double[] c, double x, ref double LeftTail, ref double Righttail)
        {
            ...
            LeftTail = NewDistrib.boost2.dist_beta(x, df2, v1, 3);
            Righttail = NewDistrib.boost2.dist_beta(x, df2, v1, 2);
        ...
        public static double BetaProdDisX2(double LeftTail, double Righttail, int p, double[] b, double[] c)
        {
            ...
            X = NewDistrib.boost2.dist_beta(LeftTail, v1, df2, 6);
            Y = 1d - X;
        ...
        public static double DavisPercentile(double f, ref double x, double LeftTail, double RightTail, double rho, double[] o)
        {
            ...
            u = NewDistrib.DistX.cdisx(LeftTail, RightTail, f);
        ...
        public static double BoxDavisGupta(int cmax, double f, double z, double rho, double[] omega)
        {
            ...
            double LogKB = 0.0d;
            double sumPDF = NewDistrib.DistMain.cdens(f, z);
            double sumCDF = NewDistrib.DistMain.cdis(f, z);
            ...
            for (int j = 1, loopTo = cmax; j <= loopTo; j++)
            {
                ...
                a[j] = temp / j;
                LogKB = LogKB + omega[j];
                double adjPDF = NewDistrib.DistMain.cdens(f + 2 * j, z);
                double adjPDF2 = a[j] * adjPDF;
                sumPDF = sumPDF + adjPDF2;
                double adjCDF = NewDistrib.DistMain.cdis(f + 2 * j, z);
        ...
        public static void NewBoxDavis(string C_Result, string C_Algorithm, int a, int b, double[] x, double[] y, double[] xi, double[] eta, ref double z, ref double LeftTail, ref double RightTail)
        {
            ...
            var loopTo2 = a;
            for (k = 1; k <= loopTo2; k++)
                sum1 = sum1 + NewDistrib.DistMain.Bernoulli(2, xi[k]) / x[k];
            sum2 = 0d;
            var loopTo3 = b;
            for (j = 1; j <= loopTo3; j++)
                sum2 = sum2 + NewDistrib.DistMain.Bernoulli(2, eta[j]) / y[j];
            rho = 1d - (sum1 - sum2) / f;
            Console.WriteLine("rho: {0}", rho);
            ...
            for (r = 1; r <= loopTo4; r++)
            {
                ...
                var loopTo5 = a;
                for (k = 1; k <= loopTo5; k++)
                    sum1 = sum1 + NewDistrib.DistMain.Bernoulli(r + 1, (1d - rho) * x[k] + xi[k]) / Math.Pow(rho * x[k], r);
                sum2 = 0d;
                var loopTo6 = b;
                for (j = 1; j <= loopTo6; j++)
                    sum2 = sum2 + NewDistrib.DistMain.Bernoulli(r + 1, (1d - rho) * y[j] + eta[j]) / Math.Pow(rho * y[j], r);
            ...
            for (r = 1; r <= loopTo7; r++)
            {
                ...
                var loopTo8 = a;
                for (k = 1; k <= loopTo8; k++)
                    sum1 = sum1 + Math.Pow(-2 * rho * x[k], r) * NewDistrib.boost2.polygamma(r - 1, x[k] + xi[k]);
                sum2 = 0d;
                var loopTo9 = b;
                for (j = 1; j <= loopTo9; j++)
                    sum2 = sum2 + Math.Pow(-2 * rho * y[j], r) * NewDistrib.boost2.polygamma(r - 1, y[j] + eta[j]);
                kappa[r] = sum1 - sum2;
            ...
            if (C_Result == "PValue") // Get p-value
            {
                ...
                NewDistrib.DistCornish.CumulantToGamma(rmax, mean, ref sigma, ref kappa, ref o);
                NewDistrib.DistCornish.CalcEdgeworth(true, false, 0, rmax - 2, (z - mean) / sigma, ref o, ref LeftTail, ref RightTail);
                Console.WriteLine("Edgeworth LeftTail {0}, RightTail: {1}", LeftTail, RightTail);
            }
            else
            {
                double x2 = NewDistrib.DistCornish.CalcCornish(LeftTail, RightTail, mean, sigma, ref kappa, rmax);
        ...
        public static double Delta(int s, int p)
        {
            ...
            var loopTo = p - 1;
            for (j = 0; j <= loopTo; j++)
                sum = sum + NewDistrib.DistMain.Bernoulli(s, -j / 2d);
            DeltaRet = -sum * (s + 1) / 2d;
            return DeltaRet;
        }
                    ...
                    case "U1DIS":
                        {
                            ...
                            for (i = 2; i <= loopTo8; i++)
                            {
                                ...
                                for (j = 0; j <= loopTo9; j++)
                                    sum = sum + NewDistrib.DistMain.Bernoulli(r + 1, (b - j) / 2d) - NewDistrib.DistMain.Bernoulli(r + 1, (b - ss[i - 1] - j) / 2d);
                            }
                    ...
                    case "LSDIS":
                        {
                            NS = NS * (n / 2d);
                            ps = ps * p;
                            d[r + 1] = (Delta(r + 1, p) + (r + 2) / 2d * NewDistrib.DistMain.Bernoulli(r + 1, 0d) / ps) / NS;
                            Beta[r] = Beta[r - 1] * b;
                            BK = r + 2;
                            sum = 0d;
                            var loopTo14 = r + 1;
        ...
        public static void Mauchlydemo()
        {
            ...
            Box(7, C_Dis, C_dfVarCount, C_dfVar, C_dfErr, C_dfErrCount, ref C_x, C_XScale, C_Algorithm, C_Result, C_LeftTail, C_RightTail);
            double p = C_dfVar[1];
            double chix = NewDistrib.DistX.cdisx(C_LeftTail, C_RightTail, (p - 1d) * (p + 2d) / 2d);
        ...
        public static void Lvc0demo()
        {
            ...
            Box(7, C_Dis, C_dfVarCount, C_dfVar, C_dfErr, C_dfErrCount, ref C_x, C_XScale, C_Algorithm, C_Result, C_LeftTail, C_RightTail);
            double p = C_dfVar[1];
            double chix = NewDistrib.DistX.cdisx(C_LeftTail, C_RightTail, p * (p + 1d) / 2d);
            Console.WriteLine("p: {0}, n: {1}, chix: {2}", C_dfVar[1], C_dfErr[1], chix);
            Console.WriteLine("X: {0}, ratio: {1}", C_x, C_x / chix);
            C_x = C_x * 1d;
            C_Result = "PValue";
        ...
        public static void Lvcmdemo()
        {
            ...
            Box(7, C_Dis, C_dfVarCount, C_dfVar, C_dfErr, C_dfErrCount, ref C_x, C_XScale, C_Algorithm, C_Result, C_LeftTail, C_RightTail);
            double p = C_dfVar[1];
            double chix = NewDistrib.DistX.cdisx(C_LeftTail, C_RightTail, p * (p + 3d) / 2d);
            Console.WriteLine("p: {0}, n: {1}, chix: {2}", C_dfVar[1], C_dfErr[1], chix);
            Console.WriteLine("X: {0}, ratio: {1}", C_x, C_x / chix);
            C_x = C_x * 1d;
            C_Result = "PValue";
