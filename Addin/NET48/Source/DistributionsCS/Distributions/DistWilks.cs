...
namespace Distributions
{
    ...
    static class DistWilks
    {
        ...
        public static void BetaflintodDis2(int p, double[] b, double[] c, double x, ref double LeftTail, ref double Righttail)
        {
            Console.WriteLine("In BetaflintodDis2 (Approximation by Nagarsenker): ");
            int i;
            double k;
            double s;
            double v1;
            ...
            // Righttail = dreal.dist_pbeta(1 - x, v1, df2)
            // Righttail = boost2.dist_beta(1 - x, v1, df2, 2)
            Righttail = dreal.dist_beta(v1, df2).cdf(1d - x);

            LeftTail = 1d - Righttail;
            // LeftTail = dreal.dist_pbeta(1 - x, v1, df2)
            // Righttail = 1 - LeftTail
            // Call betadis(v1, s * m + alpha, 1 - x, x, LeftTail, Righttail, density)
        }



        // {Approximation by Nagarsenker}
        public static double BetaflintodDisX2(double LeftTail, double Righttail, int p, double[] b, double[] c)
        {
            int i;
            double k;
            double s;
            ...
            double alpha;
            double X2, X, Y;
            Console.WriteLine("In BetaflintodDisX2 (Approximation by Nagarsenker): ");
            v1 = 0d;
            v2 = 0d;
            var loopTo = p;
            for (i = 1; i <= loopTo; i++)
            ...
            s = Math.Sqrt(s);
            double df2 = s * m + alpha;
            // X = dreal.dist_qbeta(LeftTail, v1, df2, True)
            // X = boost2.dist_beta(LeftTail, v1, df2, 6)
            X = dreal.dist_beta(v1, df2).qtf(LeftTail);
        ...
        public static void WilksExact2(int p, int f1, double f2, double l, ref double LeftTail, ref double Righttail)
        {
            ...
            BetaflintodDis2(p, b, c, l, ref LeftTail, ref Righttail);
        }
        ...
        public static double WilksExactX2(double LeftTail, double Righttail, int p, int f1, double f2)
        {
            ...
            return BetaflintodDisX2(LeftTail, Righttail, p, b, c);
        }
        ...
        // Next j
        // Call BetaflintodDis2(p, b, c, l, LeftTail, Righttail)
        // End Sub
        ...
        // 'Console.WriteLine("j: {0}, b(j): {1}, c(j): {2}", j, b(j), c(j))
        // Next j
        // Return BetaflintodDisX2(LeftTail, Righttail, p - 1, b, c)
        // End Function
        ...
        // Next j
        // Next i
        // Call BetaflintodDis2(m, b, c, l, LeftTail, Righttail)
        // End Sub
        ...
        // Next j
        // Next i
        // Return BetaflintodDisX2(LeftTail, Righttail, m, b, c)
        // End Function
        ...
        public static double GammaP(long p, double x)
        {
            double GammaPRet = 0;
            ...
            for (i = 1L; i <= loopTo; i++)
                ...
                prod = prod * dreal.real_gamma(x - 0.5d * (i - 1L));
            k = Math.Pow(pi, p * (p - 1L) / 4d);
            GammaPRet = k * prod;
            return GammaPRet;
        }
        ...
        public static double LnGammaP(long p, double x)
        {
            const double pi = 3.14159265358979d;
            long i;
            double sum;
            ...
            var loopTo = p;
            for (i = 1L; i <= loopTo; i++)
                sum = sum + DistMain.LnGamma(x - 0.5d * (i - 1L));
            // K = Log(pi ^ (p * (p - 1) / 4))
            k = Math.Log(pi) * (p * (p - 1L) / 4d);
        ...
        public static double LnHypergeometric1F1Matrix(long p, double a, double b, double[] x)
        {
            ////double k;
            double[] y;
            double tau;
            ...
            y = new double[(int)(p + 1)];
            ////prod = 1d;
            var loopTo = p;
            for (i = 1L; i <= loopTo; i++)
            {
                tau = b - x[(int)i];
                y[(int)i] = 2d * a / (tau + Math.Sqrt(tau * tau + 4d * a * x[(int)i]));
                // Prod = Prod * (((y(i) / a) ^ a) * (((1 - y(i)) / (b - a)) ^ (b - a)) * Exp(x(i) * y(i)))
                sum = sum + (Math.Log(y[(int)i] / a) * a + Math.Log((1d - y[(int)i]) / (b - a)) * (b - a) + x[(int)i] * y[(int)i]);
            ...
            for (i = 1L; i <= loopTo1; i++)
            {
                var loopTo2 = p;
                for (j = i; j <= loopTo2; j++)
                    r11 = r11 * (y[(int)i] * y[(int)j] / a + (1d - y[(int)i]) * (1d - y[(int)j]) / (b - a));
            }
            // K = b ^ (p * b - p * (p + 1) / 4)

            LogK = Math.Log(b) * (p * b - p * (p + 1L) / 4d);
        ...
        public static void TestHypergeometric1F1Matrix()
        {
            ...
            a = 61d;
            b = 2d;
            x = new double[(int)(p + 1)];
            x[1] = 1.34d;
            x[(int)p] = 2.72d;
            Result = Hypergeometric1F1Matrix((int)p, a, b, x);
            Console.WriteLine("Result: {0}", Result);
        }
        ...
        public static void TestHypergeometric0F1Matrix()
        {
            ...
            x[1] = 1.34d;
            x[(int)p] = 2.72d;
            Result = Hypergeometric0F1Matrix((int)p, n, x);
            Console.WriteLine("Result: {0}", Result);
        }
        ...
        public static void DemoUdisx()
        {
            ...
            // n: # of cases-1 }      
            int p, q, n;
            double LeftTail, RightTail, resultX, resultM, Left1 = 0, Right1 = 0;
            ...
            WilksExact2(p, q - 1, n - q, resultX, ref Left1, ref Right1);
            Console.WriteLine("WilksExact2: {0}", Left1);

            double resultWX = WilksExactX2(LeftTail, RightTail, p, q - 1, n - q);
            Console.WriteLine("resultWX: {0}", resultWX);

            // Dim WilksdisLeft1 = Wilksdis(p, q, n - q, resultWX)
            double WilksdisLeft1 = Wilksdis(p, q - 1, n - q, resultWX);
        ...
        public static double Udisx(double LeftTail, double Righttail, double p, double q, double n)
        {
            ...
            // F = dreal.dist_qf(LeftTail, pq, m, True)
            // F = boost2.dist_fisher_f(LeftTail, pq, m, 6)
            F = dreal.dist_fisher_f(pq, m).qtf(LeftTail);
        ...
        private static double B3(double h)
        {
        ...
        public static void Fangdis(int pp, int qq, double n, double l1, ref double LeftTail, ref double Righttail)
        {
            ...
            for (i = 1; i <= loopTo3; i++)
            {
                var loopTo4 = p[i];
                for (j = 1; j <= loopTo4; j++)
                    sum1 = sum1 + B3(delta - j + 1d);
            }
            var loopTo5 = b;
            for (i = 1; i <= loopTo5; i++)
                sum2 = sum2 + B3(delta - i + 1d);
            Ar2 = -(sum1 - sum2) / (2 * 3);
            if (Ar2 == 0d)
                s2 = 1d;
            else
        ...
        public static void WilksExact(int p, int f1, double f2, double l, ref double LeftTail, ref double Righttail)
        {
            ...
            BetaflintodDis(p, b, c, l, ref LeftTail, ref Righttail);
        }
        ...
        public static void BetaflintodDis(int p, double[] b, double[] c, double x, ref double LeftTail, ref double Righttail)
        {
        ...
        public static void BetaflintodDis5(bool first, bool odd_f1, int p, double[] b, double[] c, double x, double LeftTail, double Righttail)
        {
            const int Rmax = 100;
            //const int pmax = 10;
        ...
        public static void WilksExactN2(bool IsRho, int p, int f1, double f2, double l, double lambda, double LeftTail, double Righttail)
        {
            ...
            ck = c[1];
            n2 = c[1];
            // {  BetaflintodDis2(p,b,c,L,LeftTail,RightTail)}
            BetaflintodDis5(true, IsOdd_f1, p, b, c, l, LeftTail, Righttail);
            sum = LeftTail;
            if (lambda > 0d)
                RelError = 1d;
            else
            ...
            while (RelError > 0.0000000000000001d)
            {
                ...
                // {    BetaflintodDis2(p,b,c,L,LeftTail,RightTail)}
                BetaflintodDis5(false, IsOdd_f1, p, b, c, l, LeftTail, Righttail);
