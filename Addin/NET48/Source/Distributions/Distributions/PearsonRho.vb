...
#End Region
    ...
    Sub DemoFisher_kappa_X_2()
        ...
        Dim LeftMpfr As New Mpfr
        LeftMpfr = RhoExplicit_Mpfr(n, mreal.t(r), mreal.t(rho))
        Console.WriteLine("LeftTail M: {0}", LeftMpfr)



        Dim LeftArb As New Arb
        LeftArb = RhoExplicit_Arb(n, aflint.t(r), aflint.t(rho))
...
Module DemoPearsonMpfr




    Function RhoExplicit_Mpfr(n As Integer, r As Mpfr, rho As Mpfr) As Mpfr
        Dim F As New MpfrMat(), d As New MpfrMat()
        Dim sum1 As New Mpfr, sum2 As New Mpfr, sum3 As New Mpfr, sum31 As New Mpfr, sum32 As New Mpfr
        Dim X As New Mpfr, x2 As New Mpfr, r2 As New Mpfr, Rho2 As New Mpfr, U As New Mpfr
        Dim A2 As New Mpfr, a As New Mpfr, c2 As New Mpfr, C As New Mpfr, b2 As New Mpfr, b As New Mpfr
        Dim d1 As New Mpfr, f6 As New Mpfr, f6u As New Mpfr, result As New Mpfr
        Dim k As Integer, k1 As Integer, k4 As Integer
        Dim Pi As New Mpfr
        Pi = mreal.pi
        r2 = r * r : Rho2 = rho * rho
        X = r * rho : x2 = X * X
        A2 = 1 - Rho2 : a = mreal.sqrt(A2)
        c2 = 1 - r2 : C = mreal.sqrt(c2)
        b2 = 1 - x2 : b = mreal.sqrt(b2)
        U = mreal.acos(-X) / b
        F.Resize(n + 1, 1)
        d.Resize(n + 1, 1)
...
Module DemoPearsonArb




    Function RhoExplicit_Arb(n As Integer, r As Arb, rho As Arb) As Arb
        Console.WriteLine("In RhoExplicit_Arb, n:{0}", n)
        Console.WriteLine(" r: {0}, rho: {1}", r, rho)
        Dim F As New ArbMat(), d As New ArbMat()
        Dim sum1 As New Arb, sum2 As New Arb, sum3 As New Arb, sum31 As New Arb, sum32 As New Arb
        Dim X As New Arb, x2 As New Arb, r2 As New Arb, Rho2 As New Arb, U As New Arb
        Dim A2 As New Arb, a As New Arb, c2 As New Arb, C As New Arb, b2 As New Arb, b As New Arb
        Dim d1 As New Arb, f6 As New Arb, f6u As New Arb, result As New Arb
        Dim k As Integer, k1 As Integer, k4 As Integer
        Dim Pi As New Arb
        Pi = aflint.pi
        r2 = r * r : Rho2 = rho * rho
    ...
    Function RhoDensityDirect(n As Integer, r As Arb, rho As Arb) As Arb
        Dim d As New ArbMat()
        d.Resize(n + 1, 1)
        Dim Pi = aflint.pi
        Dim r2 = r * r
        ...
        Dim x2 = X * X
        Dim A2 = 1 - Rho2
        Dim a = aflint.sqrt(A2)
        Dim c2 = 1 - r2
        Dim C = aflint.sqrt(c2)
        Dim b2 = 1 - x2
        Dim b = aflint.sqrt(b2)
        Dim U = aflint.acos(-X) / b
        d(3) = A2 * (1 + X * U) / (Pi * b2 * C)
        d(4) = A2 * a * (b2 * U + 3 * X * (1 + X * U)) / (b2 * b2 * Pi)
    ...
    Function Acb_RhoDensityDirect(n As Integer, r As ArbC, rho As ArbC) As ArbC
        Dim d As New ArbMatC()
        d.Resize(n + 1, 1)
        Dim Pi = aflint.pi
        Dim r2 = r * r
        Dim Rho2 = rho * rho
        ...
        Dim x2 = X * X
        Dim A2 = 1 - Rho2
        Dim a = aflintc.sqrt(A2)
        Dim c2 = 1 - r2
        Dim C = aflintc.sqrt(c2)
        Dim b2 = 1 - x2
        Dim b = aflintc.sqrt(b2)
        Dim U = aflintc.acos(-X) / b
        d(3) = A2 * (1 + X * U) / (Pi * b2 * C)
        d(4) = A2 * a * (b2 * U + 3 * X * (1 + X * U)) / (b2 * b2 * Pi)
