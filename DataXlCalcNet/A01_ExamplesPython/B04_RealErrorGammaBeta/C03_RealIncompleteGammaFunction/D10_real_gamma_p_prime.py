...
def main_tests():
    demo_real_gamma_p_prime()


def demo_real_gamma_p_prime():
    print('demo_real_gamma_p_prime: ')
    alist = [1.5, 3.5]
    xlist = [5.1, 8.8]
    for a in alist:
        for x in xlist:
            for ctx in ctxlist:
                ...
                res = ctx.real_gamma_p_prime(a, x)
                print(name + ': gamma_p_prime(a=' + str(a) + ', x=' + str(x),'): ' + ctx.fmt(res))
            print()
        print()
