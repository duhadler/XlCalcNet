...
def main_tests():
    demo_real_rising_factorial()


def demo_real_rising_factorial():
    print('demo_real_rising_factorial: ')
    xlist = [5.1, 8.8]
    nlist = [15.1, 2.8]
    for x in xlist:
        for n in nlist:
            for ctx in ctxlist:
                ...
                res = ctx.real_rising_factorial(x, n)
                print(name, ': rising_factorial(x=' + str(x) + ', n=' + str(n),'): ', ctx.fmt(res))
            print()
        print()
