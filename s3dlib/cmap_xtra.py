...
from xlcalcnet.colorspacious import cspace_converter
...
def show_dualcmaps( plt, dcmaps, onlyColormaps=True, usize=1, show=True) :
    """
    Construct and show Matplotlib figures of dual colormaps.

    Parameters
    ----------
    plt : matplotlib.pyplot

    dcmaps : List of dual colormaps.
        List contains dual colormap. Note, name strings
        are not be used.

    onlyColormaps : bool {True,False}, optional, default: True
        If True, only a color scale figure is shown.  If False,
        the L* gray scale and L* line contours figures are also shown.

    usize : float, optional, default: 1.0
        Relative size of each colormap.
    ...
    """
    # ===================================================================
    def trans_dcmap(x,y,dcmap,Lstr=None) :
        # ...............................................
        def get_checkerboard(x,y,Lstr) :
            bgrd, blck = [1.0,0.7,0.7], [0.6,1.0,0.6]
            if Lstr is None :
                bgrd, blck = [1.0,1.0,1.0], [0.8,0.8,0.8]
            N = int(12*usize)
            arr = np.zeros((N,N,3), dtype=float)
            arr[:,:] = bgrd
            arr[1::2, ::2] = blck
            arr[::2, 1::2] = blck
            N = 0.99*N
            iX,iY = (N*x).astype(int), (N*y).astype(int)
            color = arr[iX,iY]
            return color.T
        # ...............................................
        rgbGrid = get_checkerboard(x,y,Lstr)
        colorsCmap = dcmap(x,y)
        alpha = colorsCmap[3]
        rgbCmap = colorsCmap[:3] if Lstr is None else Lstr
        delta = rgbCmap - rgbGrid
        rgb = delta*alpha + rgbGrid
        M,N = x.shape[1], y.shape[1]
        color = np.ones((4,M,N),dtype=float)
        color[:3,:,:] = rgb
        return color
    # ...................................................................
    def convert_Lstar(dcmapT) :
        rgb = dcmapT[:,:,0:3]
        L = cspace_converter("sRGB1", "CAM02-UCS")(rgb).T[0].T
        d = np.max(L)-np.min(L)
        d = d if d>=0.01 else 1  # check for uniform L*
        return (np.array(L)-np.min(L))/d
    # ===================================================================
    numb = len(dcmaps)
    ncol = 2 if onlyColormaps else 4
    # Note: following x,y length must be the same to show transparency
    y,x = np.mgrid[ 1:0:100j,0:1:100j ]   # y-reversed for imshow
    fig = plt.figure(figsize=(ncol*usize,numb*usize))
    for i,dcmap in enumerate(dcmaps):
        dcmapXY = dcmap(x,y)
        hasTransparency = np.any( np.where(dcmapXY[3]<1) )
        k = (i)*ncol + 1
        tY = 1 - i*(1/numb) - 1/(2*numb)
        tX = 0.5 if onlyColormaps else 0.25
        ax = fig.add_subplot(numb,ncol,k  )  # ........... name label
        fig.text(tX,tY,dcmap.name,ha='right',va='center',fontsize='x-large')
        ax.set_axis_off()

        ax = fig.add_subplot(numb,ncol,k+1,aspect='equal')  # ........... rgb map
        if hasTransparency :
            trmap = lambda x,y : trans_dcmap(x,y,dcmap)
            ax.imshow(trmap(x,y).T)
        else :
            ax.imshow(dcmapXY.T)
        ax.set(xticks=[],yticks=[])
        if not onlyColormaps :
            ax = fig.add_subplot(numb,ncol,k+2,aspect='equal')  # ........... L* map
            lsmap = lambda x,y : convert_Lstar(dcmapXY.T)
            if hasTransparency :
                arrlsmap = np.array(lsmap(x,y))
                mapbox = np.empty((3,*arrlsmap.shape) )
                mapbox[:] = arrlsmap.T   # note: imshow uses rows & column !!!
                trmap = lambda x,y : trans_dcmap(x,y,dcmap,mapbox)
                ax.imshow(trmap(x,y).T)
            else :
                ax.imshow(lsmap(x,y), cmap='binary_r')
            ax.set(xticks=[],yticks=[])
        if not onlyColormaps :
            ax = fig.add_subplot(numb,ncol,k+3,aspect='equal')  # ........... contours
            Lvals = convert_Lstar(dcmap(x,-y).T)
            N=7  # <--- number of contours
            minV = np.min(Lvals)
            don = (np.max(Lvals) - minV)/(N+1)
            levels = [ minV + don*(i+1) for i in range(N) ]
            ax.contour(Lvals, levels=levels , colors=['k'], linewidths=.75)
            ax.set(xticks=[],yticks=[])
