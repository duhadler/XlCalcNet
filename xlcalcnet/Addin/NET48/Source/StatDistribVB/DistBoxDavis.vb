...
#Region "GeneralAsymptotic"
    ...
    Function B3(h As Double) As Double
    ...
    Sub BetaProdDis2(p As Integer, b() As Double, c() As Double, x As Double,
                          ByRef LeftTail As Double, ByRef Righttail As Double)
        Console.WriteLine("In BetaProdDis2 (Approximation by Nagarsenker): ")
        Dim i As Integer
        Dim k As Double, s As Double, v1 As Double, v2 As Double, m As Double, alpha As Double, density As Double
        v1 = 0
        v2 = 0
        For i = 1 To p
            v1 = v1 + c(i) - b(i)
    ...
    '{Approximation by Nagarsenker}
    Function BetaProdDisX2(LeftTail As Double, Righttail As Double, p As Integer, b() As Double, c() As Double) As Double
        Dim i As Integer
        Dim k As Double, s As Double, v1 As Double, v2 As Double, m As Double, alpha As Double
        Dim X2, X, Y As Double
    ...
    Function DavisPercentile(f As Double, ByRef x As Double, LeftTail As Double, RightTail As Double, rho As Double, o() As Double) As Double
        ...
        s(7) = o(7) * P7 + o(5) * o(2) * P52 + o(4) * o(3) * P43 _
                      + 0.5 * o(3) * (o(2) ^ 2) * P322
        sum = 0
        If show Then Console.WriteLine("cdisx(f): {0}", u)
        'For i = 2 To 7
        For i = 2 To 7
            sum = sum + s(i)
    ...
    Function BoxDavisGupta(cmax As Int32, f As Double, z As Double, rho As Double, omega() As Double) As Double
        Console.WriteLine("f: {0}, z: {1}, rho: {2}", f, z, rho)
        Dim LogKB As Double = 0.0
        Dim sumPDF As Double = cdens(f, z)
        Dim sumCDF As Double = cdis(f, z)
        Dim a(100) As Double
        a(0) = 1.0
        For j = 1 To cmax
            ...
            a(j) = temp / j
            LogKB = LogKB + omega(j)
            Dim adjPDF As Double = cdens(f + 2 * j, z)
            Dim adjPDF2 As Double = a(j) * adjPDF
            sumPDF = sumPDF + adjPDF2
            Dim adjCDF As Double = cdis(f + 2 * j, z)
            Dim adjCDF2 As Double = a(j) * adjCDF
    ...
    Function BoxDavisGuptaCDF(cmax As Int32, f As Double, z As Double, rho As Double, omega() As Double) As Double
        Console.WriteLine("f: {0}, z: {1}, rho: {2}", f, z, rho)
        Dim LogKB As Double = 0.0
        Dim sumCDF As Double = cdis(f, z)
        Dim a(100) As Double
    ...
    Function BoxDavisGuptaPDF(cmax As Int32, f As Double, z As Double, rho As Double, omega() As Double) As Double
        Console.WriteLine("f: {0}, z: {1}, rho: {2}", f, z, rho)
        Dim LogKB As Double = 0.0
        Dim sumPDF As Double = cdens(f, z)
        Dim a(100) As Double
    ...
    Sub NewBoxDavis(C_Result As String, C_Algorithm As String, a As Int32, b As Int32, x() As Double, y() As Double, xi() As Double, eta() As Double, ByRef z As Double, ByRef LeftTail As Double, ByRef RightTail As Double)
        ...
        If C_Result = "PValue" Then ' Get p-value
            ...
            If C_Algorithm = "CHI2" Then
                ...
                Dim h As Double = 0.0000000001
                Dim L_1 As Double = BoxDavisGupta(rmax, f, z - h, rho, omega)
                Dim L_2 As Double = BoxDavisGupta(rmax, f, z + h, rho, omega)
                Dim DensityL As Double = (L_2 - L_1) / (2 * h)
        ...
        Else
            If C_Algorithm = "CHI2" Then ' Get percentile
                Dim Percentile = DavisPercentile(f, z, LeftTail, RightTail, rho, omega)
                Console.WriteLine("Percentile {0}", Percentile)
                Dim L_1 As Double = BoxDavisGupta(rmax, f, Percentile, rho, omega)
        ...
        For r = 1 To rmax
            sum1 = 0
            For k = 1 To a
                sum1 = sum1 + ((-2 * rho * x(k)) ^ r) * boost2.polygamma(r - 1, x(k) + xi(k))
            Next k
            sum2 = 0
            For j = 1 To b
                sum2 = sum2 + ((-2 * rho * y(j)) ^ r) * boost2.polygamma(r - 1, y(j) + eta(j))
            Next j
            kappa(r) = sum1 - sum2
    ...
    Sub NewBetaProdDist(LeftTail As Double, RightTail As Double, k As Int32, n2 As Double, bi() As Double, ci() As Double)
        ...
        'LeftTail = 0.99
        'RightTail = 1 - LeftTail
        NewBoxDavis("Quantile", "CHI2", k, k, y, y, xi, eta, z, LeftTail, RightTail)
        Console.WriteLine("z: {0}", z)
        NewBoxDavis("PValue", "CHI2", k, k, y, y, xi, eta, z, LeftTail, RightTail)
        Console.WriteLine("LeftTail: {0}", LeftTail)
    End Sub
...
#Region "New Asymptotic Tests, using BetaProdDist"
    ...
    Sub NewTestWilksU()
        ...
        Dim resultB = BetaProdDisX2(LeftTail, Righttail, p, b, c)
        Console.WriteLine("resultB: {0}", resultB)
        Dim resultL = -Math.Log(resultB)
        Console.WriteLine("resultL: {0}", resultL)
        Dim resultM = -n * Math.Log(resultB)
        Console.WriteLine("resultM: {0}", resultM)
        Call BetaProdDis2(p, b, c, resultB, LeftTail2, RightTail2)
        Console.WriteLine("LeftTail2: {0}, RightTail2: {1}", LeftTail2, RightTail2)
        Console.WriteLine()

        NewBetaProdDist(LeftTail, Righttail, p, n / 2, b, c)
    End Sub
    ...
    Sub NewTestR0DisX()
        ...
        Next j
        Dim result2 = BetaProdDisX2(LeftTail, Righttail, p - 1, b, c)
        Console.WriteLine("result2: {0}", result2)
        Dim resultM = -n * Math.Log(result2)
        Console.WriteLine("resultM: {0}", resultM)
        Call BetaProdDis2(p, b, c, result2, LeftTail2, RightTail2)
        Console.WriteLine("LeftTail2: {0}", LeftTail2)

        NewBetaProdDist(LeftTail, Righttail, p - 1, n / 2, b, c)
    End Sub
    ...
    Sub NewTestMauchly()
        ...
        Dim resultM = -n * Math.Log(result2)
        Console.WriteLine("resultM: {0}", resultM)
        Call BetaProdDis2(p - 1, b, c, result2, LeftTail2, RightTail2)
        Console.WriteLine("LeftTail2: {0}", LeftTail2)
    ...
    Sub NewTestLvcm0DisX()
        ...
        b(p) = (n - 1) / 2
        c(p) = b(p) + 1 / 2
        Dim result2 = BetaProdDisX2(LeftTail, Righttail, p, b, c)
        Console.WriteLine("result2: {0}", result2)
        Dim resultM = -n * Math.Log(result2)
        Console.WriteLine("resultM: {0}", resultM)
        Call BetaProdDis2(p, b, c, result2, LeftTail2, RightTail2)
    ...
    Sub NewTestEqualCovarianceMatricesSameSampleSize()
        ...
        Dim result2 = BetaProdDisX2(LeftTail, Righttail, m, b, c)
        Console.WriteLine("result2: {0}", result2)
        Dim resultM = -n * Math.Log(result2)
        Console.WriteLine("resultM: {0}", resultM)
        Call BetaProdDis2(m, b, c, result2, LeftTail2, RightTail2)
        Console.WriteLine("LeftTail2: {0}", LeftTail2)

        NewBetaProdDist(LeftTail, Righttail, m, n / 2, b, c)
    End Sub
...
#Region "New Asymptotic Tests, using NewBoxDavis"
    ...
    Sub NewBartlett_BoxDavis()
        ...
        For j = 1 To b
            y(j) = NN / 2
            eta(j) = (1 - j) / 2  ' Change here for equal distributions
        Next
        ...
        LeftTail = 0.95
        RightTail = 1 - LeftTail
        NewBoxDavis("Quantile", "CHI2", a, b, x, y, xi, eta, z, LeftTail, RightTail)
        Console.WriteLine("z: {0}", z)
    ...
    Sub NewEqualDistributions_Anderson()
        Dim p, q, NN, n0 As Int32
        Dim a, b, i, j, k, g As Int32
        Dim x(), y(), xi(), eta() As Double
        ...
        For j = 1 To b
            y(j) = NN / 2
            eta(j) = (q - j) / 2  ' Change here for equal distributions
        Next
    ...
    Sub NewEqualDistributions_Perlman()
        Dim p, q, NN, Ng As Int32
        Dim a, b, i, j, k, g As Int32
        Dim x(), y(), xi(), eta() As Double
        Dim N() As Int32  ' Sample sizes per group
        ...
        For j = 1 To b
            y(j) = NN / 2
            eta(j) = (0 - j) / 2  ' Change here for equal distributions
        Next
...
' Tables: Muirhead p. 514
            Case "L2DIS"
                ...
                Next i
                f = p * (p + 3) * (k - 1) / 2
                rho = 1 - ((S1 - 1 / n) * (2.0 * p * p + 3 * p - 1) / (6 * (k - 1) * (p + 3)) + (1 / n) * (p - k + 2) / (p + 3))
                mu = n * rho
                z = rho * C_x
...
' Tables: Muirhead p. 514
                Case "L2DIS"
                    ...
                    Else
                        Console.WriteLine("S2: {0}", S2)
                        omega(r) = p / (288 * rho * rho) * (6 * (S2 - 1 / (n * n)) * (p + 1) * (p - 1) * (p + 2) - (S1 - 1 / n) ^ 2 * (2 * p * p + 3 * p - 1) ^ 2 / ((k - 1) * (p + 3)) - 12 * (S1 - 1 / n) * (2 * p * p + 3 * p - 1) * (p - k + 2) / (n * (p + 3)) - 36 * ((k - 1) * (p - k + 2)) ^ 2 / (n * n * (p + 3)) - (12 * (k - 1) / (n * n)) * (-2 * k * k + 7 * k + 3 * p * k - 2 * p * p - 6 * p - 4))
                        Console.WriteLine("r: {0}, omega(r): {1}", r, omega(r))
                    End If
...
' Tables: Muirhead p. 514
                Case "L3DIS"
                    ...
                    Else
                        Console.WriteLine("S2: {0}", S2)
                        omega(r) = p * (p + 3) / (48 * rho * rho) * ((S2 - 1 / (n * n)) * (p + 1) * (p + 2) - 6 * (1 - rho) ^ 2 * (k - 1))
                        Console.WriteLine("r: {0}, omega(r): {1}", r, omega(r))
                    End If
...
' Formulas due to Davies, 1971
        ...
        If C_Result = "PValue" Then ' Get p-value
            ...
            If C_Algorithm = "CHI2" Then
                '    Call BoxDavis1(False, cmax, f, z, omega, C_LeftTail, C_RightTail)
                Call BoxDavisGupta(cmax, f, z / rho, rho, omega)
            Else
                Call BoxFApprox("PValue", f, z, omega(1), omega(2), C_LeftTail, C_RightTail)
            End If
        Else
...
#Region "Asymptotic Tests, using Old BoxDavis"
    ...
    Sub EqualDistributions_Anderson()
        Dim C_Dis As String, C_dfVarCount As Integer, C_dfVar() As Integer, C_dfErr() As Double,
        C_dfErrCount As Integer, C_x As Double, C_XScale As String, C_Algorithm As String,
        C_Result As String, C_LeftTail As Double, C_RightTail As Double, i As Int32
    ...
    Sub EqualDistributions_Perlman()
        Dim C_Dis As String, C_dfVarCount As Integer, C_dfVar() As Integer, C_dfErr() As Double,
        C_dfErrCount As Integer, C_x As Double, C_XScale As String, C_Algorithm As String,
        C_Result As String, C_LeftTail As Double, C_RightTail As Double, i As Int32
        Console.WriteLine("EqualDistributions")
