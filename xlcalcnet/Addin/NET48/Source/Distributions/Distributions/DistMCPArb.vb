...
Module DistMCPArb
    ...
    Dim AcbParams As ArbMatC = aflintc.mat_ones(100, 1)

    'Dim MpfrParams As MpfrMat = mreal_mat.set_ones(100, 1)
    Dim MpfrParams As MpfrMat = mreal.mat_ones(100, 1)

    'wolfram alpha: integrate sin(exp(x))/sqrt(x) from 0 to 2 = 1.50572...
    Function DE_Integration(func As cbFuncApc, params As ArbMatC, a As Arb, b As Arb, epsabsStart As Arb, alpha As Arb, beta As Arb) As Arb
        Console.WriteLine("DE_Integration")

        Dim ds As String = ""
        Dim p2 = aflint.pi / 2
        Dim pi = aflint.pi
        Dim K, d, C1, C2, epsabs, h, n, hmin, C1Final, epsabsFinal As New Arb
        Dim radX, radY As Double

        Dim nmin = aflint.t("1.0E1000000000000")
        '        Console.WriteLine("nmin at start: {0}", nmin)
        Dim mu, nu As New Arb
        If alpha < beta Then
            mu = alpha
            nu = beta
        Else
        ...
        For d1 As Integer = 1 To 26
            ...
            Dim radX_, radY_ As New Arb
            radX_ = aflint.t(radX) : radY_ = aflint.t(radY)
            params(mp_order) = aflintc.t(1)
            K = GetAcbK(func, params, a.Mid, b.Mid, radX_, radY_)
            params(mp_order) = aflintc.t(0)
        ...
        MM = aflint.lrint(aflint.ceil(nmin)) : NN = MM
        ...
        Dim res, sum, u, t, f, PHI2, c, b1, b2 As New Arb
        Dim x1, e1, e2, e3, fp1, fm1, su, cu, eu1, eu2 As New Arb
        Dim kk As Integer
        sum = aflint.t(0)
    ...
    Sub GetRectAndK(d1 As Integer, ByRef radX As Double, ByRef radY As Double, ByRef ds As String)
        Select Case d1
    ...
    Function GetAcbK(func As cbFuncApc, params As ArbMatC, a As Arb, b As Arb, radX As Arb, radY As Arb) As Arb
        Dim x, x1, z As New ArbC
        Dim ba2, av, x_re, x_im As New Arb
        ba2 = (b - a) / 2
        x_re.Mid = (b + a) / 2
        x_re.Rad = ba2 * radX
        x_im.Mid = aflint.t(0)
...
#If Win64 Then
    Sub WrapperParams_GL_Outer(fxPtr As IntPtr, xPtr As IntPtr, paramsPtr As IntPtr, order As UInt64, prec As UInt64)
#Else
...
#If Win64 Then
    Sub WrapperParams_GL_Inner(fxPtr As IntPtr, xPtr As IntPtr, paramsPtr As IntPtr, order As UInt64, prec As UInt64)
#Else
...
#End If
    ...
    Function RumpAcb_old(x As ArbC, params As ArbMatC) As ArbC
    ...
    Function RumpAcb(r As ArbC, params As ArbMatC) As ArbC
        Dim a = aflint.t("1000000")
        Dim z = a - 4000
        Dim f = aflintc.exp(-r)
        Dim c = (1 + f)
        'Dim d = c ^ -(a + 1)
        Dim d = aflint.pow(c, -(a + 1))
        Dim e = aflintc.exp(-z / c)
        Dim result = d * e * f
    ...
    Function AcbSinExp(t As ArbC, params As ArbMatC) As ArbC
    ...
    Public Sub DemoAcbSinExpIntegration_DE()
        ...
        Dim a, b, epsabsStart, alpha, beta As New Arb
    ...
    Function cf_chisquared(k As Arb, t As ArbC) As ArbC
        Dim ione As New ArbC
        ione = aflintc.onei
        'ione.real = 0
        'ione.imag = 1
    ...
    Function g_chisquared(t As ArbC, params As ArbMatC) As ArbC
        Dim k, x As New Arb
        k = aflint.t(10000)
        x = k - 500
        Dim result, phi, z, ione As New ArbC
        ione = aflintc.onei
        'ione.real = 0
        'ione.imag = 1
    ...
    Function g_chisquared_u2(u As ArbC, params As ArbMatC) As ArbC
        Dim t, g, result As New ArbC
        t = u / (1 - u)
    ...
    Function cf_betaflintproduct(t As ArbC, p As Int32, b As ArbMat, c As ArbMat) As ArbC
        Dim ione, result, bk, dk, g1, g2, g3, g4, prod1 As New ArbC
        result = aflintc.t(1)
        ione = aflintc.onei
        'ione.real = 0
        'ione.imag = 1
    ...
    Function g_betaflintproduct(t As ArbC, params As ArbMatC) As ArbC
        Dim n, x As New Arb
        x = aflint.t(4.5292648821553)
        Dim p As Int32 = 4
        Dim f1 As Int32 = 7 - 1
        n = aflint.t(20 - 7)
        Dim b, c As New ArbMat
        b.Resize(p + 1, 1)
        c.Resize(p + 1, 1)
        For i = 1 To p
            b(i) = ((n - i + 1) / 2)
    ...
    Function g_betaflintproduct_u2(u As ArbC, params As ArbMatC) As ArbC
        Dim t, g, result As New ArbC
        t = u / (1 - u)
    ...
    Function g_betaflintproduct_u(u As ArbC, params As ArbMatC) As ArbC
        Dim t, g, result As New ArbC
        t = (1 - u) / u
        g = g_betaflintproduct(t, params)
        result = g / (u * u)
    ...
    Sub Demo_g_betaflintproduct_GL()
        Dim p As UInt32 = 2
        'mp4.setprec(100)
    ...
    Friend Const mp_mreal_function_choice_outer_pos As Int32 = 1
    ...
    Friend Const mp_mreal_function_choice_inner_pos As Int32 = 8
    ...
    Friend Const mp_integral_g_betaflintproduct As Int32 = 25



    '**********************************************************************************************************    

    Function MpfrCalcParams_Outer(x As Mpfr, mreal_params As MpfrMat) As Mpfr
        'Dim params = aflintc.mat_t(mreal_params)
        'Dim resacb = AcbIntegrand_Outer(aflintc.t(x), params)
        Dim resapc = AcbIntegrand_Outer(aflintc.t(x), Nothing)
        Return mreal.t(resapc.real)
    End Function


    Sub FuncMpfrParams_Outer(xPtr As IntPtr, paramsPtr As IntPtr, fxPtr As IntPtr)
    ...
    Function MpfrCalcParams_Inner(x As Mpfr, mreal_params As MpfrMat) As Mpfr
        'Dim params = aflintc.mat_t(mreal_params)
        'Dim resacb = AcbIntegrand_Inner(aflintc.t(x), params)
        Dim resapc = AcbIntegrand_Inner(aflintc.t(x), Nothing)
        Return mreal.t(resapc.real)
    End Function


    Sub FuncMpfrParams_Inner(xPtr As IntPtr, paramsPtr As IntPtr, fxPtr As IntPtr)
        'Dim x As New Mpfr(xPtr, True)
        Dim fx As New Mpfr()
        'Dim params As New MpfrMat()
        'Dim tparamsPtr As IntPtr : tparamsPtr = params.mpPtr : params.mpPtr = paramsPtr
    ...
    Function AcbIntegrand_Outer(x As ArbC, params2 As ArbMatC) As ArbC
        ...
        Select Case proc_outer
            Case mp_integral_studentized_maximum : fx = Studentize(x, AcbParams)   ' 0
            Case mp_integral_studentized_maxmodulus : fx = Studentize(x, AcbParams)   ' 1
            Case mp_integral_studentized_range : fx = Studentize(x, AcbParams)   ' 3
            Case mp_integral_studentized_dunnett1 : fx = Studentize(x, AcbParams)  ' 5
            Case mp_integral_studentized_dunnett2 : fx = Studentize(x, AcbParams)   ' 7

            Case mp_integral_Rump : fx = RumpAcb(x, AcbParams)  ' 8
            Case mp_integral_g_chisquared : fx = g_chisquared_u2(x, AcbParams)  ' 24
            'Case mp_integral_g_betaflintproduct : fx = g_betaflintproduct_u2(x, AcbParams)
            Case mp_integral_g_betaflintproduct : fx = g_betaflintproduct(x, AcbParams)  '25
            Case mp_integral_AcbSinExp_DE : fx = AcbSinExp(x, AcbParams)  ' 9
            Case mp_integral_AcbSinExp_GL : fx = AcbSinExp(x, AcbParams) / aflintc.sqrt(x)  ' 10
            Case Else : Console.WriteLine("!!!! Error AcbIntegrand_Outer !!!!!)") : fx = aflintc.nan()
        End Select
    ...
    Function AcbIntegrand_Inner(x As ArbC, params2 As ArbMatC) As ArbC
        'Dim proc_inner As Int32 = AcbParams(mp_proc_inner_pos).real.ToInt32
        Dim proc_inner As Int32 = 1
        Dim fx As New ArbC
        Select Case proc_inner
            ...
            Case Else : Console.WriteLine("!!!! Error AcbIntegrand_Inner !!!!!)") : fx = aflintc.nan()
        End Select
        '        Console.WriteLine("fx: {0}", fx)
        Return fx
    End Function
    ...
    Function NdisAcb(x As ArbC) As ArbC
        Return aflintc.ndis(x)
    End Function


    Function NdensAcb(x As ArbC) As ArbC
    ...
    Function NormalMaxModulus(params2 As ArbMatC) As ArbC
        Dim res, delta As New ArbC
        'Dim proc_inner As Int32 = AcbParams(mp_proc_inner_pos).real.ToInt32
    ...
    Function AcbNormalDunnett1(y As ArbC, params2 As ArbMatC) As ArbC
        Dim rho, k, x, d As New ArbC
    ...
    Function AcbNormalDunnett2(y As ArbC, params2 As ArbMatC) As ArbC
        Dim rho, k, x, d1, d2, d As New ArbC
        k = AcbParams(mp_k_pos)
        x = AcbParams(mp_crit_inner_pos)  ' critical value for inner integration
        rho = aflintc.t("0.5")
    ...
    Function AcbNormalMCM1(y As ArbC, params2 As ArbMatC) As ArbC
        Dim rho, k, x, d As New ArbC
        k = AcbParams(mp_k_pos)
        x = AcbParams(mp_crit_inner_pos)  ' critical value for inner integration
        '        rho = aflintc.t("0.5")
    ...
    ' Multiple comparisons with the mean: see Soong 2001
    Function AcbNormalMCM2(y As ArbC, params2 As ArbMatC) As ArbC
        Dim rho, k, x, d1, d2, d As New ArbC
        k = AcbParams(mp_k_pos)
        x = AcbParams(mp_crit_inner_pos)  ' critical value for inner integration
        '        rho = aflintc.t("0.5")
    ...
    Function AcbNormalRange(y As ArbC, params2 As ArbMatC) As ArbC
        Dim k, x, d1, d2, d As New ArbC
    ...
    Function Studentize(x As ArbC, params2 As ArbMatC) As ArbC
        Dim n, c, a, b, res1, res2, fx As New ArbC
        'Dim proc_outer As Int32 = AcbParams(mp_proc_outer_pos).real.ToInt32
        'Dim k As Int32 = AcbParams(mp_k_pos).real.ToInt32
        Dim proc_outer As Int32 = 1
        Dim k As Int32 = 1
