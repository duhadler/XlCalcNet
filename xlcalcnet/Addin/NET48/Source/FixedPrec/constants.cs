...
namespace FixedPrecNet
{
    public static class constants
    {
        ...
        // note: use xcn.verbosity to deal with "quiet", "errors", "warnings", and "full" 

        // note: use xcn.apr_prec = 1 to intmax 
        // note: use xcn.apr_dps = 1 to intmax 

        // note: use xcn.zpr_format_base = 2 to 62 
        // note: use xcn.mpfr_format_base = 2 to 32 
        // note: use xcn.dbl_format_base = 2 to 16 
        // note: use xcn.apr_format_base = 2 or 10 

        // note: use xcn.apr_format_digits = 1 to +inf (=full) 
        // note: use xcn.apr_format_pretty = "disabled", "enabled"  

        // note: use xcn.chop_mode to deal with chopping values near zero: "disabled", "enabled"  
        // note: use xcn.chop_level to set the chopping threshold  


        // note: use xcn.apr_auto_mode to deal with internal precision ("none", "jointly": only abs(z), "separately": re/im ) 
        // note: use xcn.apr_radius_mode to deal with mpr routines incl. rounding: "disabled", "enabled"  
        // note: use xcn.apr_usage to control apr usage when called from Double or mpfr ("always", "backup") 
        // note: use xcn.amath_usage to control amath usage when called from Double ("always", "backup") 

        // note: use xcn.matrix_mode to deal with "scalar", "array", "matrix", "sparsematrix", "polynomial", and "series" 
        // note: use xcn.series_n to set the length of series calculations from 1 to n 

        // note: modify rint parameters  to deal with rint-rounding: "trunc", "floor", "ceil", "nearest" 
        ...
        internal const int mp_conv_set_to_complex_dbl = 350;
        internal const int mp_conv_get_from_complex_dbl = 351;
        ...
        internal const int mp_real_fft_fwd = 2;
        internal const int mp_real_fft_inv = 3;
        ...
        internal const int mp_xpc_get_d = 129;
        ...
        // BEGIN: ONLY COMPLEX 
        internal const int mp_cplxfunc0_onei = 820;
        // END: ONLY COMPLEX 
        ...
        internal const int mp_conv_mat_set_part_in_complex = 5;
        internal const int mp_conv_mat_get_part_from_complex = 6;
