...
Module PermMilton
    ...
    Function Myfunction(x As Double, j As Integer) As Double
        ...
        If IsNormal Then
            Dim res As Double
            'res = Math.Exp(-((x - delta(j)) ^ 2) / 2) * sqrt2pi
            'Console.WriteLine("MyFunc: {0}", res)
            'Return res
            'Console.WriteLine("j: {0}, delta(j): {1}", j, delta(j))
    ...
    Sub InitMilton(GroupAnz As Integer, ByRef n() As Integer)
        ...
        Console.WriteLine("delta(2): {0}", delta(2))
        Factor = 1
        p = 0
        For j = 1 To GroupAnz
            For i = 1 To n(j)
