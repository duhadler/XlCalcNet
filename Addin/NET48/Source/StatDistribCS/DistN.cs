...
namespace NewDistrib
{
    ...
    static class boost2
    {
        ...
        public static double dist_normal(double xqp, double mean, double sd, double target)
        {
            return 1d;
        }
        ...
        public static double owens_t(double a, double h)
        {
            return 1d;
        }
        ...
        public static double dist_student_t(double xqp, double df, double target)
        {
            return 1d;
        }
        ...
        public static double dist_chisq(double xqp, double df, double target)
        {
            double result = double.NaN;
            var rv = dreal.dist_chi2(df);
            if (target == 6d)
                result = rv.qtf(xqp);
            if (target == 7d)
                result = rv.sf(xqp);
        ...
        public static double dist_fisher_f(double xqp, double df1, double df2, double target)
        {
            return 1d;
        }
        ...
        public static double dist_beta(double xqp, double a, double b, int target)
        {
            double result = double.NaN;
            var rv = dreal.dist_beta(a, b);
            if (target == 2)
                result = rv.cdf(xqp);
            if (target == 3)
                result = rv.sf(xqp);
        ...
        public static double polygamma(int r, double x)
        {
    ...
    static class DistN
    {
        ...
        {
            double lnPower;
            double LeftTail; // , Righttail As Double
            LeftTail = non_central_chi_square_cdf(x1, Df1, lambda);
            lnPower = Math.Log(LeftTail);
        ...
        public static void Demo_ChiSquare_Lambda()
        {
            ...
            double alpha;
            double Beta;
            double DF1, LeftTail; // , Righttail As Double
        ...
        public static double non_central_chi_square_p(double y, double n, double lambda, double init_sum)
        {
            ...
            double gamkf = boost2.gamma_p(a, x);
            ...
            double poiskf = boost2.gamma_p_derivative(k + 1, del);
            ...
            double xtermf = boost2.gamma_p_derivative(a, x);
        ...
        public static void CdisnCohen(double n, double X, double lambda, ref double LeftTail, ref double RightTail)
        {
            ...
            double F1 = boost2.dist_normal(x1 - d, 0d, 1d, 2d) - boost2.dist_normal(-x1 - d, 0d, 1d, 2d);
            double F3 = F1 - 2d * g3;
        ...
        //    p1 = Math.Pow(2d, j - 1) * boost2.gamma(j) / Math.Pow(1d - 2d * t, j);
        ...
        public static void NonCentralChi2_SPA2(double n, double x, double lambda, ref double LeftTail, ref double Righttail)
        {
            ...
            {
                double p1 = Math.Pow(2d, j - 1) * boost2.gamma(j) / Math.Pow(1d - 2d * t, j);
                double p2 = n + lambda * j / (1d - 2d * t);
                return p1 * p2;
            }

            double K1(double t)
            {
                return CGF_Derivative(t, 1) - x;
            }
            ...
            //var res1 = dreal.NewtonRaphson(K1, K2, 0.0, -10, 0.5, 51, 50U);
            var res1 = dreal.NewtonRaphson(K1, K2, 0.0, -100, 100, 51, 50U);
            Console.WriteLine("res1 (x0, iter): {0}", res1);

            double s, density = default;
            Console.WriteLine("n: {0}, x: {1}, lambda: {2}", n, x, lambda);
            s = -(1d / (4d * x)) * (n - 2d * x + Math.Sqrt(n * n + 4d * x * lambda));

            Console.WriteLine("s: {0}", s);
            Console.WriteLine("K1(s): {0}", K1(s));
            Console.WriteLine("K2(s): {0}", K2(s));
        ...
        public static double GammaHalf(int mj)
        {
            ...
            return boost2.gamma(mj + 0.5d) / Math.Sqrt(Math.PI);
        }
        ...
        {
            double mu, w1, w2, LeftTail0 = default, RightTail0 = default, u, w;
            ...
            double lastsumj = Double.PositiveInfinity;
            for (int j = 0, loopTo3 = useorder; j <= loopTo3; j++)
            {
                sum[j] = A[j] - B[j];
                if (Math.Abs(sum[j]) < Math.Abs(lastsumj))
                {
                    lastsumj = Math.Abs(sum[j]);
        ...
        public static double non_central_beta_p(double a, double b, double lambda, double x, double y, double init_val)
        {
            ...
            double pois = boost2.gamma_p_derivative(k + 1, l2);
            if (pois == 0d)
                return init_val;
            double xterm = default, beta;
            if (x < y)
            {
        ...
        public static double non_central_t_cdf_main(double v, double delta, double t, bool invert)
        {
            ...
            if (X < cross)
            {
                ...
                else
                {
                    ...
                    result += boost2.dist_normal(-delta, 0d, 1d, 2d);
                }
            }
            else
            {
        ...
        public static double non_central_t2_p(double v, double delta, double x, double y, double init_val)
        {
            ...
            // * delta / Math.Sqrt(2)
            pois = boost2.gamma_p_derivative(k + 1, d2) * boost2.gamma_delta_ratio(k + 1, 0.5d) * delta / Math.Sqrt(2d);
        ...
        public static double TdisnOwen(long n, double X, double d, ref double LeftTail, ref double RightTail)
        {
            ...
            if (k == 0L)
                ...
                F = DistMain.ndis(-d * b) + 2d * boost2.owens_t(d * b, a);
        ...
        public static void demo_tdisn_samplesize()
        {
            double alpha;
            double Beta;
            double LeftTail = default, RightTail = default, FSign, Factor;
            bool IsExact = false;
            bool IsGLM = true;
            ...
            double x1 = boost2.dist_student_t(alpha, Df2, 6d);
            ...
            do
            {
                ...
                x1 = boost2.dist_student_t(alpha, N2, 6d);
        ...
        public static void TDisnOrRhoSquareDis(bool IsExact, bool IsGLM, double df2, double t, double delta, double omega, ref double LeftTail, ref double RightTail)
        {
            if (IsGLM)
            {
                if (IsExact)
                {
                    if (omega == 0d)
                    {
                        TdisnOwen(Convert.ToInt32(df2), t, delta, ref LeftTail, ref RightTail);
                    }
                    else
                    {
        ...
        public static void RhoDisNew(bool IsExact, bool IsGLM, double N, double r, double rho, ref double LeftTail, ref double RightTail)
        {
            if (IsGLM)
            {
                ...
                if (IsExact)
                {
                    TdisnOwen(Convert.ToInt32(N) - 2, t, delta, ref LeftTail, ref RightTail);
                }
                else
                {
                    var PDF = default(double);
        ...
        {
            double zTransformInverseRet = default;
            y = Math.Exp(2d * y);
        ...
        {
            double r, rho, nu, mu, a, b, x, nc, nc2, xbeta, ybeta, LeftTail0 = default, RightTail0 = default;
            double LeftTail1 = default, RightTail1 = default;
            double LeftTail2 = default, RightTail2 = default;
            double LeftTail3 = default, RightTail3 = default;
            var PDF = default(double);
            int dis = 1;
            mu = 16d;
            nu = 20d;
                ...
                case 4:
                    {
                        ...
                        BetadisnSeber(xbeta, a, Convert.ToInt32(b), nc, ref LeftTail0, ref RightTail0);
