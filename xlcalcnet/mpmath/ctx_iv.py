...
# change DH #
    ...
    print("x: ", x, "; str(type(x)", str(type(x)))
    if (str(type(x)) == "<class 'numpy.int64'>"):
        s = str(x)
        return from_str(s, prec, rounding)
    raise TypeError
...
class ivmpc:
    ...
    def conjugate(s):
        ...
        # change DH
        return s.ctx.make_mpc((a, mpi_neg(b)))
